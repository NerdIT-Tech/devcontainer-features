...
# Use the option from devcontainer-feature.json. Options are passed as
# environment variables named by UPPERCASING the option key.
CONTAINER_PIPE="${CONTAINERBROWSERPIPEPATH:-/tmp/hostbrowserpipe}"

# Idempotent: skip if the wrapper is already installed.
if [ -x /usr/local/bin/xdg-open ]; then
    echo "xdg-open wrapper already installed"
    exit 0
fi
...
if [ -p "${CONTAINER_PIPE}" ]; then
    ...
    if [ -f /usr/bin/xdg-open ]; then
        ...
        echo "Error: /usr/bin/xdg-open not found and ${CONTAINER_PIPE} is not a pipe." >&2
        exit 1
    fi
fi
EOF
